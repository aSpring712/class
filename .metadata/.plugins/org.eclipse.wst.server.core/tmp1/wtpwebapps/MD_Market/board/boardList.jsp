	...
	<div class="d-flex justify-content-between mt-3">
		...
		<ul class="pagination">
			...
			<c:if test="${p.currentPage != i }">
				<li class="page-item"><a class="page-link" href="boardlist?pageNum=${i }&field=${p.field}&word=${p.word}">${i }</a></li> <!-- controller에서 pageNum 받아옴 -->
			</c:if>
			<!-- 현재 페이지 -->
			<c:if test="${p.currentPage == i }">
		...
		<!-- 검색 -->
		<form class="form-inline" action="boardlist"> <!-- 비동기 함수로 callback 하지않고 바로 처리하겠다, field값 word값 들고감 -->
